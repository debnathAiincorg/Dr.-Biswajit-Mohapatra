...
async function buildAssets() {
  ...
  await esbuild({
    ...
    loader: { '.css': 'css' },
    /* base/fonts.css points at the woff2 files with relative url()s that are
       correct for the OUTPUT location, and the files themselves are
       passthrough-copied from node_modules rather than living in src/. Marking
       them external stops esbuild trying to resolve them against the source
       tree (where they do not exist) and leaves the URLs exactly as written,
       which is what keeps them working under PATH_PREFIX. */
    external: ['*.woff2'],
  });
...
export default function (eleventyConfig) {
  ...
  /* The Gallery's video and its poster frame. Separate from images/ because a
     10MB .mp4 is not an image and does not belong in the same sweep. */
  eleventyConfig.addPassthroughCopy({ 'src/assets/video': 'assets/video' });
  /* site.webmanifest is NOT copied: it is generated by src/webmanifest.ts, so
     that the paths inside it get the pathPrefix applied. See that file. */

  /*
   * Self-hosted webfonts, copied out of the @fontsource packages rather than
   * committed into src/. npm owns the version (pinned in package.json), and
   * dist/ is the only place the binaries ever land.
   *
  ...
  for (const file of [
    ...
    eleventyConfig.addPassthroughCopy({
      [`node_modules/@fontsource/inter/files/${file}`]: `assets/fonts/${file}`,
    });
  }
  eleventyConfig.addPassthroughCopy({
