...
- Design tokens and the type scale live in `src/assets/css/base/tokens.css` and
  ...
  > **Superseded 2026-09-25: the burger menu is now used at every width, and
  > opens a right-hand off-canvas drawer.** At the site owner's request the
  > inline nav row was removed and the header shows only the wordmark and the
  > burger. The drawer (`.nav-drawer`, with a blurred `.nav-scrim` behind it)
  > holds the 13 links and replaces the old drop-down `.mobile-panel`. It sits
  > **after `</header>`** in `partials/header.ts`, deliberately: the header's
